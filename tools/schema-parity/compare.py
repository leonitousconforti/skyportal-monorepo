...
def kind_of(schema: dict[str, Any], defs: dict[str, Any]) -> tuple[str, bool]:
    ...
    if variants:
        ...
        if not kinds:
            return "null", True
        return (kinds[0] if len(kinds) == 1 else "union<" + "|".join(kinds) + ">"), nullable
...
def reduce_model(schema: dict[str, Any]) -> dict[str, Any]:
    ...
    return {"extra": schema.get("additionalProperties", True) is not False, "fields": fields}


def main() -> int:
    """Compare the two dumps and report."""
    py = {k: reduce_model(v) for k, v in json.loads((OUT / "py.json").read_text()).items()}
    ts = {k: reduce_model(v) for k, v in json.loads((OUT / "ts.json").read_text()).items()}
    allow = set(json.loads(ALLOWLIST.read_text()).get("differences", [])) if ALLOWLIST.exists() else set()

    differences: list[str] = []
    for name in sorted(set(py) & set(ts)):
        a, b = py[name], ts[name]
        if a["extra"] != b["extra"]:
            differences.append(f"{name}: additional properties py={a['extra']} ts={b['extra']}")
    ...
    print(f"compared {len(set(py) & set(ts))} models; {len(py_only)} python-only, {len(ts_only)} typescript-only")
    if ts_only:
        print("\ntypescript-only models (add the Python side):")
        print("\n".join(f"  {n}" for n in ts_only))
    if py_only:
    ...
    return 1 if unexpected or ts_only or stale else 0
