...
describe("unwrap", () => {
    it("returns the data field of a success envelope", async () => {
        const response = new Response(
            JSON.stringify({ status: "success", data: { id: 1 } })
        );
    ...
    it("reports a non-JSON body rather than throwing a parse error", async () => {
        ...
        await expect(Http.unwrap(response)).rejects.toThrow(
            "SkyPortal returned a non-JSON response (HTTP 502)"
        );
    });

    it("treats a 200 that is not a success envelope as an error", async () => {
        const response = new Response(
            JSON.stringify({ status: "error", message: "bad input" })
        );
...
describe("params", () => {
    it("drops null and undefined but keeps false and zero", () => {
        expect(
            Http.params({ a: 1, b: undefined, c: null, d: false, e: 0, f: "" })
        ).toEqual({
            a: 1,
            d: false,
            e: 0,
